...
class App {
  ...
  _DeleteWorkout(e) {
    ...
    for (let i = 0; i < liElements.length; i++) {
      if (Number(liElements[i].attributes['data-id'].value) === Number(e.target["attributes"].value.value)) {
        liElements[i].remove();
      }
    }

    for (let item of this.#workouts) {
      if (Number(item.id) === Number(e.target.attributes.value.value)) {
        lat1 = item.coords[0];
        lng1 = item.coords[1];
    ...
    this.#mapLayers.forEach((p) => {
      if (p._latlng['lat'] == latlng['lat'] && p._latlng['lng'] == latlng['lng'])
        currentmarker = p;
    });
  ...
  showToasterMessage(message, type) {
    ...
    if (type == 0) {
      toasterContainer.style = 'background-color:orangered;'
    } else if (type == 1) {
      toasterContainer.style = 'background-color:#1bdf87;'
    }
    ...
    setTimeout(() => {
      ...
      if (toasterContainer.childElementCount === 0) {
        toasterContainer.style.display = 'none';
      }
    }, 3000);
  }

  // toast = document.querySelector('p-toast');
  // showToast(message) {
  //   toast.show({
  //     severity: 'info',
  //     summary: 'Message',
